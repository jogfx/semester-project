...
def fetch_data_from_api(api_key):
    ...
    api_url = f"https://newsapi.org/v2/everything?q=*&from=10-05-2024&to={previous_day}&sortBy=popularity&apiKey={api_key}"
...
def preprocess_data(data, article_retriever):
    # Example preprocessing, modify according to your needs
    ...
    df['Content'] = df.apply(lambda row: article_retriever.preprocess(row['Name']['name'], row['url']), axis=1)
    return df
...
# %%
 # Save preprocessed data to vector database
client = chromadb.PersistentClient(path="/work/MLops")
